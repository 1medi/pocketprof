...
export default function Page() {
  ...
  const onAnswerSelected = (answer, idx, event = null) => {
    if (event && event.type === 'keydown' && event.key !== 'Enter' && event.key !== ' ') {
      return; // Only proceed for Enter or Space keydown, ignore other keys
    }
    setChecked(true);
          ...
          <div className={styles.quizContainer}>
            ...
            {answers.map((answer, idx) => (
              <li
                ...
                tabIndex={0}> {/* tabIndex changed to 0 */}
                <span>{answer}</span>
              </li>
            ))}
            {checked ? (
              <button onClick={nextQuestion} className={styles.btn} tabIndex={2}>
